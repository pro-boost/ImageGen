    ...
    >
      <div className="relative overflow-hidden">
        <img
          src={imageUrl}
          alt={prompt}
          className={`w-full ${height || "h-48"} object-cover transition-transform duration-300 group-hover:scale-105`}
        />
      </div>
      
      <div className="p-4">
        <h3 className={`${width ? "text-xs" : "text-sm"} font-medium text-gray-500 uppercase tracking-wide mb-1`}>
          Prompt
        </h3>
        <p className={`${width ? "text-sm" : "text-base"} text-gray-800 font-medium leading-relaxed line-clamp-3`}>
          {prompt}
        </p>
      </div>
